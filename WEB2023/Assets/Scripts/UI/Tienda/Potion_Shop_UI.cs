...
public class Potion_Shop_UI : MonoBehaviour
{
    ...
    void AddShopEvents()
    {
        ...
        if (tutorialData.GetComponent<NPCHelperManager>().tutorialPociones)
        {
    ...
    void GenerateShopItems()
    {
        ...
        for (int i = 0; i < itemDB.Potions_Shop_Items_Count; i++)
        {
            //Shop_Item shop_Item = itemDB.GetItemToPool(i);
            UpgradeSO shop_Item = itemDB.PotionShopGetItemFromDB(i);
    ...
    {
        UpgradeSO shop_Item = itemDB.PotionShopGetItemFromDB(index);
        Item_UI ui_item = GetItem_UI(index);

        if (player.GetComponent<CoinCounter>().totalMoney >= itemDB.PotionShopGetItemFromDB(index).ShopValue)
        {
            // Compra el objeto y resta el dinero
            Debug.Log("Purchased" + index);
            player.GetComponent<CoinCounter>().totalMoney -= itemDB.PotionShopGetItemFromDB(index).ShopValue;
            player.GetComponent<CoinCounter>().UpdateTotalMoneyText();
            itemDB.PotionPurchaseUpgrades(index);
            ...
            if (!tutorialData.GetComponent<NPCHelperManager>().tutorialPociones)
            {
                tutorialData.GetComponent<NPCHelperManager>().tutorialPociones = true;
                closeShopButton.gameObject.SetActive(true);
            }
