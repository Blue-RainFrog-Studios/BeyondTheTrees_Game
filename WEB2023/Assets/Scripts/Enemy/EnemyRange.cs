...
public class EnemyRange : EnemyController
{
    ...
    void Update()
    {
        ...
        if (!notInRoom)
        {
            ...
            if (Vector3.Distance(transform.position, player.transform.position) < attackRange && Vector3.Distance(transform.position, player.transform.position) > range && !room.GetComponent<RoomController>().lowHealth() && currState != EnemyState.Heal)
            {
                currState = EnemyState.Attack;

            }
             else if (Vector3.Distance(transform.position, player.transform.position) < attackRange && Vector3.Distance(transform.position, player.transform.position) > range && room.GetComponent<RoomController>().lowHealth() || room.GetComponent<RoomController>().healing)
            {
